...
The `resume-mcp` server is a dedicated service within the HireHawk architecture that manages and exposes candidate profiles, resumes, and career details for candidate **Gokul**. It is designed to expose candidate credentials to downstream agent workflows.
