...
export default function LoginSignup({ onLoginSuccess }: LoginSignupProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      localStorage.setItem('neurohire_auth', 'true');
      localStorage.setItem('neurohire_user_email', email);
      localStorage.setItem('neurohire_user_name', isLogin ? 'Demo Recruiter' : name);
      localStorage.setItem('neurohire_user_role', role);
  ...
  const handleSocialLogin = (provider: string) => {
    ...
    setTimeout(() => {
      ...
      setSuccess(true);
      localStorage.setItem('neurohire_auth', 'true');
      localStorage.setItem('neurohire_user_email', `sso_${provider.toLowerCase()}@neurohire.ai`);
      localStorage.setItem('neurohire_user_name', `SSO User (${provider})`);
      localStorage.setItem('neurohire_user_role', 'recruiter');
      setTimeout(() => {
        onLoginSuccess(`sso_${provider.toLowerCase()}@neurohire.ai`);
      }, 1200);
    }, 1000);
  };
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  ...
                  <input
                    type="email"
                    placeholder="gokul@neurohire.ai"
                    value={email}
