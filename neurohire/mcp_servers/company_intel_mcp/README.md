...
- **Integrations**:
  ...
  - **Upstash Redis Semantic Caching**: Configured via `NeuroHireSemanticCache` in `main.py`. Wraps `get_company_intel` to check semantic proximity of requests against past runs before invoking external calls. Falls back gracefully when Upstash is unconfigured.
