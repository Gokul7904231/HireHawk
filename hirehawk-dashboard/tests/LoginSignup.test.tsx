...
describe('LoginSignup Component Tests', () => {
  test('renders login form by default and can toggle to signup form', () => {
    ...
    // Default: Login mode
    expect(screen.getByText('HireHawk Workspace')).toBeInTheDocument();
    expect(screen.getByText('Log in to orchestrate your autonomous recruitment pipeline')).toBeInTheDocument();
    expect(screen.getByPlaceholderText('gokul@hirehawk.ai')).toBeInTheDocument();
  ...
  test('handles successful login and triggers callback', async () => {
    ...
    const emailInput = screen.getByPlaceholderText('gokul@hirehawk.ai');
    const passwordInput = screen.getByPlaceholderText('••••••••');
    const submitBtn = screen.getByRole('button', { name: /Login Workspace/i });

    // Fill in valid credentials
    fireEvent.change(emailInput, { target: { value: 'candidate@hirehawk.ai' } });
    ...
    await waitFor(() => {
      expect(handleLoginSuccess).toHaveBeenCalledWith('candidate@hirehawk.ai');
    }, { timeout: 2000 });
  });
});
